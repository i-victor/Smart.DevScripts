...
//==
define('APPCODEPACK_VERSION',  'v.171122.1705'); 								// current version of this script
...
header('Expires: '.gmdate('D, d M Y', @strtotime('-1 year')).' 05:09:00 GMT'); 	// HTTP 1.0
...
function RunApp() {
	...
	//--
	echo '<!DOCTYPE html>'."\n".'<html>'."\n".'<head><meta charset="UTF-8"><title>Smart'.'::'.'AppCode.PACK</title><style type="text/css">*{font-family:arial,sans-serif;font-smooth:always;} hr{border:none 0; border-top:1px solid #CCCCCC; height:1px;} a{color:#000000 !important;}</style></head>'."\n".'<body>'."\n";
	echo '<table><tr>';
			...
			if(AppPackUtils::is_type_file((string)APPCODEPACK_COMPRESS_UTILITY_MODULE_JS)) {
				if(AppPackUtils::have_access_read((string)APPCODEPACK_COMPRESS_UTILITY_MODULE_JS)) {
					...
					switch((string)APPCODEPACK_COMPRESS_UTILITY_TYPE) {
						case 'U': // node uglify
							$parr = (array) AppPackUtils::run_proc_cmd(
								(string) escapeshellcmd((string)APPCODEPACK_COMPRESS_UTILITY_BIN).' '.APPCODEPACK_COMPRESS_UTILITY_MODULE_JS.' '.trim((string)APPCODEPACK_COMPRESS_UTILITY_OPTIONS_JS).' --screw-ie8 --beautify beautify=false,ascii-only=true -- '.escapeshellarg($y_script_path),
								null,
								(string) __DIR__,
								null
...
final class AppPackUtils {
	...
	// ::
	// v.171007 {{{SYNC-CLASS-APP-PACK-UTILS}}}
