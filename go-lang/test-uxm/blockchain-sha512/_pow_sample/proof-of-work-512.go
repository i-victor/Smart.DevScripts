...
const (
	difficulty uint64 = 12 // 4 * zeroesPrefixNum
)
...
func verifyHash(target *big.Int, hdrHash []byte, nonce int64) (ok bool, bytes []byte) {
	...
	testHash := sha512.Sum512(testNum.Bytes())
...
//	fmt.Printf("target: %x\n", target)
	...
	log.Println("[DEBUG] Target: " + hex.EncodeToString(target.Bytes()))
	hdrHash := generateHeaderHash(data, prevHash)
//	fmt.Printf("Header-Hash: %x\n", hdrHash)
	fmt.Println("Header-Hash: " + hex.EncodeToString(hdrHash))
	var nonce int64 = -1
	for nonce = 0; nonce < math.MaxInt64; nonce++ {
		testVerify, theBytes := verifyHash(target, hdrHash, nonce)
		fmt.Printf("\rProof-of-Work: %s (Nonce: %d)", hex.EncodeToString(theBytes), nonce)
		if(testVerify == true) {
			fmt.Println("")
...
func main() {
	...
	for i = 0; i < max; i++ {
		...
		d = smart.JsonEncode(d) // ISSUE: json encode may vary ... and the block header will vary accordingly ; TODO: add data as base64#Line1 \n base64#line2 ...
		b = NewBlock(d, prev)
		fmt.Printf("Id: %s\nData: %s\nHash: %s\nNonce: %d\nPrevious: %s\n########\n",
			b.Id,
			b.Data,
	...
	log.Println("[DATA] Block Hex Hash:", hex.EncodeToString(b.Hash))
	log.Println("[DATA] Block Prev Hex Hash:", hex.EncodeToString(b.PrevHash))
	j := &JsonBlock {
		b.Id,
		b.DateTime,
		b.CheckSum,
		...
		smart.Base64Encode(string(b.Hash)),
		smart.Base64Encode(string(b.PrevHash)),
		smart.Base64Encode(b.Data),
	}
	var jsonStr string = smart.JsonEncodePretty(j)
//	var jsonStr string = smart.JsonEncode(j)
	...
	B := &Block{
		...
		[]byte(smart.Base64Decode(D["hash"].(string))),
		[]byte(smart.Base64Decode(D["prevhash"].(string))),
