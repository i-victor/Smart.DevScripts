...
func (m *JSContext) EvalCode(code string) string {
	inputLen := C.size_t(len(code))
	filename := C.CString("code")
	evalFlags := C.int(0)
	val := C.JS_Eval(m.ref, C.CString(code), inputLen, filename, evalFlags)
	strval := "result: " + C.GoString(C.JS_ToCString(m.ref, val))
	return strval
}
