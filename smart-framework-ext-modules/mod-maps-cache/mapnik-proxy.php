...
class SmartAppIndexController extends SmartAbstractAppController {
	...
	public function Run() {
		...
		if((string)$cfg_custom_mapnik != '') {
			$r = ''.$cfg_custom_mapnik; // 'custom-mapnik' :: force mapnik (caching cost a lot of space, so we just do this trick ...)
		} //end if
		//==
		...
		if((string)SMART_FRAMEWORK_DEBUG_MODE != 'yes') {
			...
			//--
			$parsed_domain = @gethostbyname($parsed_referer['host']);
			...
			if(\SmartUtils::check_ip_in_range('127.0.0.1', '127.0.0.255', ''.$parsed_domain) != 1) {
			...
			if(((string)$cfg_custom_iprange_start != '') AND ((string)$cfg_custom_iprange_end != '')) {
				if(\SmartUtils::check_ip_in_range(''.$cfg_custom_iprange_start, ''.$cfg_custom_iprange_end, ''.$parsed_domain) != 1) {
