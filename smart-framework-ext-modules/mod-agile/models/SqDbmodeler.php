<?php
// Class: \SmartModDataModel\Agile\SqDbmodeler
...
final class SqDbmodeler {

	// ->
	// v.170825

	private $db = null;
	private $sqdb = '#db/dbmodeler.sqlite3';
	...
	public function __construct() {
		...
		if(!\SmartFileSystem::is_type_file((string)$this->sqdb)) {
			...
			throw new \Exception('DBMODELER: DB SQLITE File does NOT Exists !');
			return;
		} //end if
		//--
		if(!$this->initDBSchema()) {
			throw new \Exception('DBMODELER: Failed to Initialize the DbModeler Data Table !');
			return;
		} //end if
		//--
	} //END FUNCTION
	...
	public function getOneByUuid($uuid) {
		...
		//--
		return (array) $this->db->read_asdata('SELECT * FROM `db_modeler` WHERE (`uuid` = ?) ORDER BY `id` DESC LIMIT 1 OFFSET 0',
			[
				(string) $uuid
			]
		);
	...
	public function getAllByUuid($limit=100) {
		...
		return (array) $this->db->read_adata(
			'SELECT * FROM `db_modeler` GROUP BY `uuid` ORDER BY `id` DESC LIMIT '.(int)$limit.' OFFSET 0'
		);
		//--
	} //END FUNCTION
	...
	public function saveData($data, $user='') {
		...
		$wr = $this->db->write_data(
			'INSERT INTO `db_modeler` '.$this->db->prepare_statement(
				(array) $newdata,
				'insert'
			)
		);
...
//--
	...
	private function checkInstance() {
		//--
		if(!$this->db instanceof \SmartSQliteDb) {
			throw new \Exception('Invalid DBMODELER DB Connection !');
			return 0;
		} //end if
		//--
		return 1;
	...
	private function initDBSchema() {
		...
		//--
		if($this->db->check_if_table_exists('db_modeler') != 1) { // create table if not exists
			//--
			$this->db->write_data("CREATE TABLE 'db_modeler' ('id' INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, 'dtime' character varying(23) NOT NULL, 'uuid' character varying(32) NOT NULL, 'title' character varying(255) NOT NULL, 'saved_data' TEXT NOT NULL, 'user' character varying(96) NOT NULL); CREATE INDEX 'uuid' ON 'db_modeler' ('uuid');");
			//--
		} //end if
		//--
		if($this->db->check_if_table_exists('db_modeler') != 1) { // create table if not exists
