...
item:
	...
	attributes:
		...
		height:
			...
			max: 6000
			decimals: 0
			validation: number-list-integer
			validhint:
...
//print_r($attributes); die();
					$arr[] = [
						'id' 		=> $val['id'],
						'name' 		=> $this->parseItemName($val['name']),
						'price' 	=> $val['price'],
						'currency' 	=> $this->cart_currency,
						'img-src' 	=> $val['image']['source'],
						'img-w' 	=> $val['image']['width'],
						'img-h' 	=> $val['image']['height'],
						'atts' 		=> $attributes,
						'hash' 		=> $cart->calculateHash($val['id'], $attributes)
					];
				} //end foreach
			} //end if
			//--
			...
			foreach($attribs as $k => $v) {
				if(is_array($v['values'])) {
					...
					foreach($v['values'] as $kk => $vv) {
						if(is_array($vv)) {
							$price = $vv['price'] ? (string)' '.$vv['price'].' '.$this->cart_currency : '';
							$attvalues[(string)$vv['id']] = ($vv['hint'] ? (is_array($vv['hint']) ? (string)$vv['hint']['en'] : (string)$vv['hint']) : (string)$vv['id']).$price;
						} //end if
					} //end foreach
					$attributes[$k] = (array) $attvalues;
				} //end if
			} //end foreach
		} //end if
		//--
